...
export function AssetTemplateMetrics({ assetValue, monthlyMaintenanceCost, monthlyIncome = 0 }) {
  ...
  return (
    <MetricsRow>
      ...
      <MetricInlineItem tip="Обслуживание за ход (списание в конце хода; в модели — ежемесячная сумма)" glyph="down" tone="neg">
        <MoneyText value={monthlyMaintenanceCost} decimals={0} />
      </MetricInlineItem>
      {showIncome ? (
        <MetricInlineItem tip="Доход за ход (начисление в конце хода; в модели — ежемесячная сумма)" glyph="up" tone="pos">
