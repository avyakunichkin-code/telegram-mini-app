/** Тексты подсказок по целям победы (без React). */

export function guidanceForGoal(goal, view) {
...
/** Однострочная подсказка для свёрнутого блока «Цель». */
export function buildGoalActionHint(goal, view) {
  const copy = guidanceForGoal(goal, view);
  return (copy.lead || [])
    .map((part) => (part.highlight != null ? part.highlight : part.text ?? ''))
    .join('')
    .trim();
}
