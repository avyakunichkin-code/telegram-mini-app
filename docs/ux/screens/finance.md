...
| **С главной** — «Весь капитал →» / «Вложить» | Таб `finance` без якоря; «Вложить» — цель онбординга ведёт сюда для депозита/облигаций |
...
|-------------|----------|----------------------|
| **Вход** | `BottomGameNav` → Капитал | `activeTab = finance` |
| **Вход** | Dashboard chip Доходы/Расходы | `finance` + `capitalFlowsOpen` = income \| expense |
| **Вход** | Dashboard «Весь капитал →» | `finance`, flows без якоря |
...
1. **Ориентация** (hero) — «Капитал», подзаголовок с доступными разделами (без pill «N разделов»).  
...
|------|-----------|------------|
| **Z0 Hero** | `MqxTabHero` `mqx-hero--capital` | title «Капитал», без `sectionLabel`; subtitle из `capitalPageSubtitle(mechanics)` |
...
1. При `activeTab = finance` отображается hero «Капитал» и блоки Доходы/Расходы, если есть `overview`.
