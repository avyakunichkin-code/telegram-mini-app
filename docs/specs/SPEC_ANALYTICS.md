...
| 4 | **Динамика** | Тёмная карта (как prod): **①** две линии доход/расход по закрытым ходам; **②** ликвидность — история слева, прогноз справа, 3/6/12 на графике |
| 5 | **Рекомендации** | 1 главная + до 2 вторичных (`MqxInsightCard`) |
| 6 | **Победа** *(Game, optional)* | Одна строка / ссылка «цели на главной» |

**Графики (канон после lab A ★, 2026-06-21):**

- Водопад месяца — **структура текущего снимка**; график доход/расход — **факт по закрытым периодам** (не дублировать).
- Прогноз ликвидности — паттерн `CashForecastSpark`: **одна** polyline (хвост timeseries + projected от текущей точки); слева ломаная, справа прямая; горизонт **3 | 6 | 12** меняет правую часть одного SVG.
...
| Sparklines / split-line SVG | Одна линия ликвидности (история→прогноз); доход+расход — две линии на одном поле |
| Segmented forecast | 3/6/12 — длина правой части графика ликвидности |
