...
|---------|------------|-------------|
| **`mechanics` / `mechanics_unlock`** | Открытие разделов капитала по шаблону и целям победы | [ADR-004](../../decisions/ADR-004-mechanics-unlock-victory-chain.md); **не** через достижения |
| **Victory v2** | Победа партии (chain или parallel) по шаблону | Общие метрики; **разные** движки и JSON |
| ~~**character_level** / XP~~ | **Снято** | [ADR-003](../../decisions/ADR-003-remove-character-progression.md) |
| **События `xp_delta`** | В БД может остаться; **игнорируется** движком | ADR-003 |
