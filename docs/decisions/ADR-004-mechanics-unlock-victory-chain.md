...
После Victory v2 (ADR-002) и tutorial-цепочки (`action_once` в `victory_config_json`) игрокам нужен **предсказуемый порядок** открытия разделов вкладки **«Капитал»**: сначала потоки, затем подушка → инвестиции → страховки → недвижимость.
