(function () {
  ...
  function setExpanded(block, expanded) {
    ...
    const toolBtn = block.closest('.lab-variant-card')?.querySelector('[data-toggle-expand]');
    if (toolBtn) {
      toolBtn.classList.toggle('is-on', expanded);
      toolBtn.textContent = expanded ? 'Свернуть' : 'Развернуть';
    }
  }
  ...
  function bindBlock(block) {
    ...
    const expandBtn = block.closest('.lab-variant-card')?.querySelector('[data-toggle-expand]');
    if (expandBtn) {
