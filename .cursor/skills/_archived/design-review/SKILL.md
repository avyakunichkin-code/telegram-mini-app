...
Перед любой записью в репозиторий (GDD, review-log, systems-index): **May I write** / «Могу записать …?» — дождись ответа пользователя.

Use a second `AskUserQuestion`:
- Prompt: "May I write updates to `design/gdd/systems-index.md` to mark [system] as [In Review / Approved]?"
